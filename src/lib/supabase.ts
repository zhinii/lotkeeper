...
export function publicPhoto(path: string | null) {
  ...
  return supabase.storage.from("public-records").getPublicUrl(path).data.publicUrl;
}

export async function throwIfError<T extends { error: { message: string } | null }>(result: T) {
