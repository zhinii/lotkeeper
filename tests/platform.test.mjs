...
test("public browsing combines image, text and filter search with the map", async () => {
  ...
  assert.match(directory, /activeResult\.photo_path/);
  assert.match(directory, /SKU \/ asset ID/);
  assert.match(directory, /Open full details/);
  assert.match(directory, /focusFinder/);
  assert.match(map, /sku \? `SKU \$\{sku\}`/);
  assert.match(planMap, /sku \? `SKU \$\{sku\}`/);
  assert.match(styles, /\.finder-toolbar/);
  assert.match(styles, /\.active-result-section/);
  assert.match(styles, /\.active-result-card/);
...
test("submitters choose optional AI after photo and EXIF preparation", async () => {
  ...
  assert.match(submit, /Photo EXIF/);
  assert.match(submit, /Live phone GPS/);
